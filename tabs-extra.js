...
function renderEmiTab() {
    ...
    const emiPct = ((totalEmi / DATA.salary) * 100).toFixed(1);
...
function renderAffordTab() {
    ...
    const bikePct = ((bikeTotal / sal) * 100).toFixed(1);
    const totalWithBike = currentSpend + bikeTotal;
    const totalWithBikePct = ((totalWithBike / sal) * 100).toFixed(1);
    ...
    wf.innerHTML = items.map(item => {
        const barW = Math.min(100, Math.abs(item.value / sal) * 100);
    ...
    recHtml += `<div class="remain-item" style="border-left:3px solid #6366f1"><span class="remain-label">📊 Current total obligations: ${fmt(currentSpend)} (${((currentSpend/sal)*100).toFixed(0)}% of salary). With bike: ${fmt(totalWithBike)} (${totalWithBikePct}%).</span></div>`;
